...
```shell
oc label node tsbhvspnguatw1.arabbanking.local node-role.kubernetes.io/pnguat=""  
oc label node tsbhvspnguatw2.arabbanking.local node-role.kubernetes.io/pnguat=""     
```

## Taint nodes (Optional)

```shell
oc adm taint node tsbhvspnguatw1.arabbanking.local dedicated=pnguat:NoSchedule
oc adm taint node tsbhvspnguatw2.arabbanking.local dedicated=pnguat:NoSchedule
```

## Create New project or namespace

```shell
oc new-project pnguat
```
...
```shell
# 1. Create SA
oc apply -f bootstrap/cluster/pnguat-sa.yaml

# 2. Create SCC
oc apply -f bootstrap/cluster/pnguat-scc.yaml

# 3. Bind SCC to SA 
oc apply -f bootstrap/cluster/pnguat-scc-bind.yaml

# 4. Apply Custom role
oc apply -f bootstrap/cluster/pnguat-role.yaml

# 5. Apply Custom role-bind
oc apply -f bootstrap/cluster/pnguat-rolebind.yaml
...
spec:
  ...
  resources:
    requests:
      storage: 100Gi # Adjust size based on your requirements
...
```bash
oc apply -f oc-registry/registry-pvc.yaml
```
...
oc apply -f oc-registry/configure-registry-storage.yaml
...
# If same namespace this happening, can ignore this
...
oc apply -f oc-registry/imagepuller-binding.yaml
